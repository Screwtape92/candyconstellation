...
export function bakeSpriteCanvas(
  ...
  if (visual.draw === 'star') {
    ...
    // Bakes the art rotated rather than rotating the live sprite at runtime,
    // so entities that use this texture (the Sour Blaster ship reskin) keep
    // a constant `angle` of 0 like everything else — see SpriteVisual's
    // `rotate180` doc comment.
    if (visual.rotate180) {
      ctx.save()
